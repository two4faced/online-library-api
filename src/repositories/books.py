...
class BooksRepository(BaseRepository):
    ...
    async def get_books_with_genres(self, *filters, **filter_by):
        ...
        return [
            BookWithGenresDTO.model_validate(model, from_attributes=True) for model in result.scalars().all()
        ]

    async def get_one_book_with_genres(self, **filter_by):
        query = (
            select(self.model).options(selectinload(self.model.genres)).filter_by(**filter_by)
        )
        result = await self.session.execute(query)
